...
function renderDateCells() {
    var div = createElement('div', {
        className: 'fzk-dp-cells'
    })

    var date = this._visibleDate
    var dateStr =
        date.getFullYear() + '/' + padDate(date.getMonth() + 1) + '/' +
            padDate(date.getDate())

    var selected = this.options.date
    var selectedStr =
        selected.getFullYear() + '/' + padDate(selected.getMonth() + 1) +
            '/' + padDate(selected.getDate())

    var now = new Date()
    var nowStr =
        now.getFullYear() + '/' + padDate(now.getMonth() + 1) + '/' +
            padDate(now.getDate())
...
function resolveSelector(sel) {
    this.container = (typeof (sel) === 'string') ? $$(sel) : sel;
    ...
    if (this._elms.input) {
        ...
        this._elms.floater = this.container = createElement('div', {
            className: 'fzk-dp-float'
        });
...
function parseDate(str, format) {
    ...
    var keys = {}, i = 1,
        ...
        if (!date || date.length !== 4 || !keys.day || !keys.month ||
            !keys.year
        ) {
            return null;
        }
    ...
    if (date[keys.year].length === 2) {
        date[keys.year] = (date[keys.year] < 25 ? '20' : '19') +
            date[keys.year];
    }
